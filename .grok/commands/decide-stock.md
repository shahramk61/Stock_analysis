...
6. Emit FINAL TRANSACTION PROPOSAL + **full decision JSON** (`policy_conflict`, `policy_action`, `debate_path`, `pipeline_refs`, …).  
   If Research BUY and Execute FLAT, set `policy_conflict: true` and do **not** force long.
7. Save `decisions/live_<TICKER>_<timestamp>.json`.
