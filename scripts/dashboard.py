    ...
    with tabs[5]:
        ...
        if "error" not in multi and "horizons" in multi:
            ...
            if sel_h and sel_h in horizons:
                ...
                # === LSTM (green dashed) — v4.18: realistic path with trend + noise ===
                ...
                try:
                    ...
                    lstm_dyn = lstm  # fall back to cached result
                if lstm_dyn and "all_predictions" in lstm_dyn:
                    raw_preds = lstm_dyn["all_predictions"]
                    total_ret = sum(raw_preds) / 100.0
                    n = max(len(raw_preds), horizon_days)
                    np.random.seed(42)
                    noise = np.random.normal(0, 0.8, n)
                    trend = np.linspace(0, total_ret * 100, n)
                    realistic = np.clip(trend + noise, -4, 4)
                    cum_lstm = np.cumsum(realistic).tolist()[:len(daily) if daily else n]
                ...
                # === Chronos-2 (orange dotted) — v4.19: dynamic call with selected horizon ===
                chronos_dyn = None
                try:
                    from signals import get_chronos_forecast as _ch_fn
                    chronos_dyn = _ch_fn(ticker, prediction_length=horizon_days)
                except Exception:
                    chronos_dyn = chronos  # fall back to cached result
