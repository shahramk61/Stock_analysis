...
def get_chronos_forecast(ticker: str, prediction_length: int = 5, use_covariates: bool = True):
    """
    Chronos-2 forecast. With `use_covariates=True` and pandas_ta available, feeds
    [Close, Volume, RSI, ATR] as a multivariate context (shape: 1, 4, history_length).
    Falls back to univariate Close-only on any covariate or library failure.

    Chronos2Pipeline.predict returns a list of tensors, each with shape
    (n_variates, 21_quantile_levels, prediction_length). Target is at variate 0.
    Quantile indices: q05=1, q10=2, q50=10, q90=18.
    """
    try:
        ...
        if use_covariates:
            try:
                ...
                if len(df) >= 60:
                    ...
                    arr = context_df.values.T.astype(np.float32)            # (4, 120)
                    ctx_t = torch.from_numpy(arr).unsqueeze(0)               # (1, 4, 120)
        ...
        # Univariate fallback / default
        if forecast_tensor is None:
            close = hist['Close'].dropna().values[-100:].astype(np.float32)
            last_price = float(close[-1])
            ctx_t = torch.from_numpy(close).unsqueeze(0).unsqueeze(0)        # (1, 1, 100)
            forecast = pipeline.predict(ctx_t, prediction_length=prediction_length)
            forecast_tensor = forecast[0].cpu().numpy()                       # (1, 21, h)

        # Extract Close-target quantiles (target is at variate index 0)
        target_q = forecast_tensor[0]   # (21, prediction_length)
        q10_arr = target_q[2]
        q50_arr = target_q[10]
        q90_arr = target_q[18]
